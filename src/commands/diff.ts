...
type RemoveContentType = Record<string, unknown>;
...
interface Field extends ContentFields<KeyValueMap>, Record<string, unknown> {}
...
interface ModifyEditorInterface {
	name: string;
	field: string;
	oldControl: Control;
	newControl: Control;
...
interface ModifyDisplayField {
	oldDisplayField: string;
	newDisplayField: string;
}

interface ContentfulChange {
	type: ChangeType;
	contentType: string;
...
export function command(program: Command): Command {
	program
		...
		.action(async (lowerEnvName, higherEnvName, options) => {
			...
			if (lowerEnvName === 'master') {
				throw new Error(`${chalk.red('[ERROR}')} lower env cannot be master`);
			}

			// fetch higher and lower environments
			const progress = ora(`Contentful env diff ${chalk.cyan(lowerEnvName)} -> ${chalk.yellow(higherEnvName)}`).start();
...
export async function getContentfulEnvironment(env: Environment): Promise<ContentfulEnvironment> {
	let progress = ora(`Fetching ${chalk.cyan('content types')} for ${chalk.cyan(env.name)}`).start();
	const contentTypesList = await getContentTypes(env);
	progress.succeed(`Got ${chalk.yellow(contentTypesList.length)} types from ${chalk.cyan(env.name)}`);

	progress = ora(`Fetching ${chalk.cyan('editor interfaces')} in environment ${chalk.cyan(env.name)}`).start();
	const collection = await env.getEditorInterfaces();
	const editorInterfacesList = collection.items;
	progress.succeed(`Got ${chalk.yellow(editorInterfacesList.length)} editor interface in environment ${chalk.cyan(env.name)}`);
 ...
 */
export function diffObject(a: string | object | number | boolean | undefined, b: string | object | number | boolean | undefined): string {
	const at = typeof a;
	const bt = typeof b;
